...
processed_blocks = normalize_blocks(blocks[:200])
...
t_bar = np.sum(processed_blocks, axis=0)/len(processed_blocks)
o_t = np.sqrt(np.sum((processed_blocks - t_bar)**2, axis=0))
...
for bnum in range(0, 16):
    ...
    for kguess in range(0, 256):
        ...
        # Add your code here
        # ###################
        hws = np.array([[HW[aes_internal(textin[bnum],kguess)] for textin in plaintexts[:200]]]).transpose()

        hws_bar = mean(hws)
        o_hws = std_dev(hws, hws_bar)
        correlation = cov(processed_blocks, t_bar, hws, hws_bar)
